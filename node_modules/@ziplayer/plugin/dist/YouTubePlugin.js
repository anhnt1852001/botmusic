...
const sabr_stream_factory_1 = require("./utils/sabr-stream-factory");
const stream_converter_1 = require("./utils/stream-converter");
/**
...
class YouTubePlugin extends ziplayer_1.BasePlugin {
    ...
    async init() {
        this.client =
            this.options.client ??
                (await youtubei_js_1.Innertube.create({
    ...
    buildTrack(raw, requestedBy, extra) {
        ...
        const duration = pickFirst(raw?.length_seconds, raw?.duration?.seconds, raw?.duration?.text, raw?.duration, raw?.length_text, raw?.basic_info?.duration);
    ...
    async search(query, requestedBy) {
        ...
        if (this.validate(query)) {
            ...
            const res = await this.searchClient.search(videoId, {
                type: "video",
            });
    ...
    async getStream(track) {
        ...
        try {
            ...
            const sabrOptions = { ...sabr_stream_factory_1.DEFAULT_SABR_OPTIONS };
            const { stream, title, format } = await (0, sabr_stream_factory_1.createSabrStream)(id, this.client, sabrOptions);
            ...
            if (stream && typeof stream.getReader === "function") {
                ...
                const nodeStream = (0, stream_converter_1.webStreamToNodeStream)(stream, 32 * 1024); // Optimized buffer size
            ...
            return {
                stream: (0, stream_converter_1.webStreamToNodeStream)(stream, 32 * 1024),
                type: "arbitrary",
                metadata: track.metadata,
            };
        }
